...
void TITULOS();
int MENU();
void CREAR(int MATRIZ[10][10], int VECTOR[10], int &FIL, int &COL, int &N);
void CONSULTA(int MATRIZ[10][10], int VECTOR[10], int FIL, int COL, int N);
...
main()
  {
  char OP = 'S';
  int  TIPO, F = 0, C = 0, N = 0, MTZ[10][10], VEC[10];
  while ( OP == 'S')
    {
    TITULOS();
    TIPO = MENU();
    switch (TIPO) {
       case 1 : CREAR(MTZ, VEC, F, C, N);
                break;
       case 2 : CONSULTA(MTZ, VEC, F, C, N);
                break;
       case 3 : OP = 'N';
       }
    }
...
void TITULOS()
  ...
  system("cls");
  system("color 04");
  gotoxy(16,5) ; cout <<"CREAR MATRIZ Y VECTOR POR FILAS";
  gotoxy(18,6);  cout <<"SUMA DE CADA FILA DE LA MATRIZ" ;
  }
...
void CONSULTA(int MATRIZ[10][10], int VECTOR[10], int FIL, int COL, int N)
  ...
  if ( FIL == 0)
    ...
    {
    TITULOS();
    gotoxy(22,9) ; cout << "IMPRIMIR MATRIZ Y VECTOR" ;
    ...
    F = 14 + FIL;
    gotoxy(10,F); cout << "VECTOR SUMA FILAS =";
    gotoxy(32,F);
    for ( K = 0 ; K < N ; K++ )
      cout << VECTOR[K] << "   ";
