...
void CONSULTA(int VECTOR[15], int N);
int ELIMINAR( int VECTOR[15], int N);
  ...
  while ( CONTROL == 'S')
     ...
     switch   (OPCION)  {
        case 1 : NRO = CAPTURA(VECTOR, NRO);
                 break;
        case 2 : CONSULTA( VECTOR, NRO);
               	 break;
        case 3 : NRO = ELIMINAR( VECTOR,NRO);
               	 break;
        case 4 : CONTROL = 'N';
           };
       }
...
int ELIMINAR( int VECTOR[15], int N )	
  ...
  if ( N <= 0 )
     ...
     gotoxy(44,15); VALOR = VALNUMERO(45,15,10);
     SW = 0 ;	
	 for ( K = 0 ; K < N ; K++ ) 
	     if (VALOR == VECTOR[K])
	        {
	        ...
	        for ( I = K ; I < N ; I++ ) 
	            VECTOR[I] = VECTOR[I+1];
	        N-- ; 
	        gotoxy(28,20); cout << "OK, Elemento Eliminado en la Lista";
            }
	 if (SW == 0)
	    {
	    gotoxy(28,20); cout << "Elemento NO Existe en la Lista ---> " << VALOR;	
	    }	  
     getch();
     return N;
     }	
