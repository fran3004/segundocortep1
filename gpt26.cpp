...
void CREAR(int MATRIZ[10][10], int INVERSA[10][10], int FIL, int COL);
...
main()
  {
  char OP = 'S';
  int  TIPO, F = 0, C = 0, MTZ[10][10], INVERSA[10][10];
  while ( OP == 'S')
    {
    TITULOS();
    TIPO = MENU();
...
void TITULOS()
  ...
  system("cls");
  system("color F1");
  gotoxy(13,5) ; cout <<"CAPTURAR MATRIZ Y CREAR MATRIZ INVERSA";
  gotoxy(19,6);  cout <<"FILAS POR COLUMNAS - COLUMNAS POR FILAS" ;
  }
...
int MENU()
  ...
  gotoxy(20,11); cout << "2 - CREAR E IMPRIMIR MATRIZ INVERSA";
...
void CREAR(int MATRIZ[10][10], int INVERSA[10][10], int FIL, int COL)
  ...
  if ( FIL == 0)
    ...
    {
    TITULOS();
    gotoxy(17,9) ; cout << "MATRIZ ORIGINAL Y MATRIZ INVERSA" ;

    for ( I = 0 ; I < FIL ; I++ )
      for ( J = 0 ; J < COL ; J++ )
        INVERSA[J][I] = MATRIZ[I][J];

    gotoxy(10,12); cout << "MATRIZ ORIGINAL =";
    for ( I = 0 ; I < FIL ; I++ )
      {
      gotoxy(28,12 + I);
      for ( J = 0 ; J < COL ; J++ )
    ...
    F = 14 + FIL;
    gotoxy(10,F); cout << "MATRIZ INVERSA  =";
    for ( I = 0 ; I < COL ; I++ )
      {
      gotoxy(28,F + I);
      for ( J = 0 ; J < FIL ; J++ )
    ...
    gotoxy(10,F+COL+3); cout << "Pulse cualquier tecla para salir ";
    getch();
    }
  }
