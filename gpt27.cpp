...
void CALCULO(int MATRIZ[10][10], int MENOR[100], int MAYOR[100], int FIL, int COL);
...
main()
  {
  char OP = 'S';
  int  TIPO, F = 0, C = 0, MTZ[10][10], MENOR[100], MAYOR[100];
  while ( OP == 'S')
    {
    TITULOS();
    TIPO = MENU();
    switch (TIPO) {
       case 1 : CREAR(MTZ, F, C);
                break;
       case 2 : CALCULO(MTZ, MENOR, MAYOR, F, C);
                break;
       case 3 : OP = 'N';
       }
    }
...
void TITULOS()
  ...
  system("cls");
  system("color F1");
  gotoxy(12,5) ; cout <<"CREAR MATRIZ Y VECTORES MENOR Y MAYOR";
  gotoxy(18,6);  cout <<"ELEMENTOS SEGUN EL PROMEDIO" ;
  }
...
int MENU()
  ...
  gotoxy(20,11); cout << "2 - IMPRIMIR MATRIZ Y VECTORES";
...
void CALCULO(int MATRIZ[10][10], int MENOR[100], int MAYOR[100], int FIL, int COL)
  ...
  if ( FIL == 0)
    ...
    {
    TITULOS();
    gotoxy(15,9) ; cout << "MATRIZ Y VECTORES OBTENIDOS" ;
    SUMA = 0;
    NMENOR = 0;
    NMAYOR = 0;
    ...
    for ( I = 0 ; I < FIL ; I++ )
      for ( J = 0 ; J < COL ; J++ )
        if ( MATRIZ[I][J] < PROMEDIO )
           {
           MENOR[NMENOR] = MATRIZ[I][J];
           NMENOR++;
           }
         else
           {
           MAYOR[NMAYOR] = MATRIZ[I][J];
           NMAYOR++;
           }

    F = 14 + FIL;
    gotoxy(10,F); cout << "PROMEDIO = " << PROMEDIO;

    gotoxy(10,F+2); cout << "VECTOR MENOR =";
    gotoxy(27,F+2);
    for ( K = 0 ; K < NMENOR ; K++ )
      cout << MENOR[K] << "   ";

    gotoxy(10,F+4); cout << "VECTOR MAYOR =";
    gotoxy(27,F+4);
    for ( K = 0 ; K < NMAYOR ; K++ )
      cout << MAYOR[K] << "   ";

    gotoxy(10,F+6); cout << "ELEMENTOS MAYOR O IGUAL AL PROMEDIO = " << NMAYOR;
    gotoxy(10,F+9); cout << "Pulse cualquier tecla para salir ";
    getch();
    }
  }
