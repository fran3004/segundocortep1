...
void CALCULO(int MATRIZ[10][10], int PAR[100], int IMPAR[100], int FIL, int COL);
...
main()
  {
  char OP = 'S';
  int  TIPO, F = 0, C = 0, MTZ[10][10], PAR[100], IMPAR[100];
  while ( OP == 'S')
    {
    TITULOS();
    TIPO = MENU();
    switch (TIPO) {
       case 1 : CREAR(MTZ, F, C);
                break;
       case 2 : CALCULO(MTZ, PAR, IMPAR, F, C);
                break;
       case 3 : OP = 'N';
       }
    }
...
void TITULOS()
  ...
  system("cls");
  system("color F1");
  gotoxy(15,5) ; cout <<"CREAR MATRIZ Y VECTORES PAR E IMPAR";
  gotoxy(18,6);  cout <<"ELEMENTOS PARES E IMPARES" ;
  }
...
int MENU()
  ...
  gotoxy(20,11); cout << "2 - CALCULAR E IMPRIMIR MATRIZ Y VECTORES";
...
void CALCULO(int MATRIZ[10][10], int PAR[100], int IMPAR[100], int FIL, int COL)
  ...
  if ( FIL == 0)
    ...
    {
    TITULOS();
    gotoxy(17,9) ; cout << "IMPRIMIR MATRIZ Y VECTORES PAR E IMPAR" ;
    NP = 0;
    NI = 0;
    ...
    F = 14 + FIL;
    gotoxy(10,F); cout << "VECTOR PAR   =";
    gotoxy(25,F);
    for ( K = 0 ; K < NP ; K++ )
      cout << PAR[K] << "   ";

    gotoxy(10,F+2); cout << "VECTOR IMPAR =";
    gotoxy(25,F+2);
    for ( K = 0 ; K < NI ; K++ )
      cout << IMPAR[K] << "   ";
