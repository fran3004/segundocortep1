...
void TITULOS();
int MENU();
void CREAR(int MATRIZ[10][10], int VECTOR[100], int &FIL, int &COL, int &N);
void CONSULTA(int MATRIZ[10][10], int VECTOR[100], int FIL, int COL, int N);
...
main()
  {
  char OP = 'S';
  int  TIPO, F = 0, C = 0, N = 0, MTZ[10][10], VEC[100];
  while ( OP == 'S')
    {
    TITULOS();
    TIPO = MENU();
...
void TITULOS()
  ...
  system("cls");
  system("color 04");
  gotoxy(18,5) ; cout <<"CREAR MATRIZ Y OBTENER VECTOR";
  gotoxy(18,6);  cout <<"ELEMENTOS DE UNA MATRIZ 10 X 10" ;
  }
